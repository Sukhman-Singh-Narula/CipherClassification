...
for k, v in results.items():
    ...
    print(f"{k}: acc={v['test_acc']:.4f} n_test={n_test} p_vs_1/3={pval:.3g} CI95=({ci.low:.4f},{ci.high:.4f})")

with open("data/binom_results.json","w") as f:
...
with open("data/summary_multiseed.json") as f:
    summary = json.load(f)
with open("data/binom_results.json") as f:
    binom = json.load(f)

rows = []
for L in [8,24,32,64,1024]:
    for m in ['ANN','DNN','1D-CNN','FFNN']:
        k = f"{m}_{L}"
        s = summary[k]; b = binom[k]
        rows.append({
            "Model": m, "Length_bytes": L,
            "Mean_Acc_3seed": round(s["mean_acc"],4), "Std_3seed": round(s["std_acc"],4),
            "Collapse_Frac_Dominant_Class": round(s["collapse_frac_to_dominant_class"],3),
            "n_test": b["n_test"], "p_value_vs_1_3_chance": round(b["p_value_vs_1_3"],4),
            "CI95_low": round(b["ci_95"][0],4), "CI95_high": round(b["ci_95"][1],4),
            "n_samples_per_cipher": 12000 if (L==1024 and m in ("ANN","1D-CNN")) else 131072,
        })
